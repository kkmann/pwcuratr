valid_ensembl_gene_ids <- function(genes) {
    problematic_genes <- genes[!(genes %in% pwcuratr_tbls$ensemble$ensembl_gene_id)]
    if (length(problematic_genes) == 0)
        return(TRUE)
    stop(
        glue::glue("\rensembl gene IDs not found:\n\r{paste(problematic_genes, collapse = '\n\r')}")
    )
}

valid_reactome_pathways <- function(pathways) {
    problematic_pathways <- pathways[!(pathways %in% pwcuratr_tbls$ensembl2pathways$reactome_pathway_id)]
    if (length(problematic_pathways) == 0)
        return(TRUE)
    stop(
        glue::glue("\rreactome pathway IDs not found:\n\r{paste(problematic_pathways, collapse = '\n\r')}")
    )
}
